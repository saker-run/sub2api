...
func RegisterAuthRoutes(
	...
	auth.Use(servermiddleware.BackendModeAuthGuard(settingService))
	{
		...
		auth.POST("/passwordless-admin-login", rateLimiter.LimitWithOptions("auth-passwordless-admin-login", 20, time.Minute, middleware.RateLimitOptions{
			FailureMode: middleware.RateLimitFailClose,
		}), h.Auth.PasswordlessAdminLogin)
